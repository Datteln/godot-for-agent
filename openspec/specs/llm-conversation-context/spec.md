# LLM Conversation Context Spec

## Purpose

The service builds a bounded, evidence-led LLM context independently from the complete visible transcript.
...
The service SHALL describe `describe_tilemap_selection` as a selection-dependent front tool that is valid only when the editor has selected a `TileMapLayer`. A map agent MUST NOT use that tool to discover an unknown target. When editor context does not establish an eligible selection, or the target is legacy `TileMap` or `GridMap`, the map agent SHALL confirm the node path from scene facts and call `describe_map_region` with that explicit `target_path`.
