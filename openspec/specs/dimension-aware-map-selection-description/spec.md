# Dimension-Aware Map Selection Description Spec

## Purpose

Describe the currently selected map node — `TileMapLayer`, legacy `TileMap`, or `GridMap` — with its resolved path, concrete node type, and map dimension, and provide dimension-matched bounded-region parameters so subsequent region inspection and screenshot capture match the selected dimension.
