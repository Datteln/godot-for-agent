# Three-Dimensional Map Region Capture Spec

## Purpose

Capture a bounded `GridMap` cell cuboid through the 3D viewport with an explicit dimension-specific input contract, camera-state restoration, and honest evidence-scope reporting, while preserving the existing 2D TileMap map-region behavior.
