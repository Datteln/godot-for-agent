...
## FastAPI / Pydantic / SQLAlchemy Typing

```python
from pydantic import BaseModel, ConfigDict
from sqlalchemy.orm import Mapped, mapped_column

# API schema: only for request/response contracts
class UserOut(BaseModel):
    id: int
    nickname: str
    model_config = ConfigDict(from_attributes=True)

# ORM model: only for persistence
class UserModel:
    id: Mapped[int] = mapped_column(primary_key=True)
    nickname: Mapped[str]

def to_user_out(model: UserModel) -> UserOut:
    """将 ORM 实体映射为对外响应模型。"""
    return UserOut.model_validate(model)
```

要点：
- 不要直接返回 ORM 对象给 API 层，统一映射到 Pydantic schema。
- `from_attributes=True` 可减少手写字段拷贝并保持类型安全。
- SQLAlchemy 2.0 推荐使用 `Mapped[T]` / `mapped_column` 明确字段类型。
...
建议：
- 优先安装类型桩（stubs）而不是直接放宽检查。
- 仅对确实缺失类型信息的依赖启用 `ignore_missing_imports`。
- 将 overrides 写成最小范围，避免掩盖真实类型问题。
