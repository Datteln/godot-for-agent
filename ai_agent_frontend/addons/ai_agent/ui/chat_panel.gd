...
核心原则：凡是地图规划/编辑/校验工具，只要缺少当前 `target_path + map_layer + map_revision + affected region`，就不能继续执行，必须先读地图。
...
```text
如果缺 target_path / map_layer / map_revision / region_context：
  自动插入 describe_map_region
  读取完成后，把结果写入 session map_context_state
  然后恢复原工具调用var _user_is_dragging_scrollbar := false   # 用户正在拖拽滚动条
