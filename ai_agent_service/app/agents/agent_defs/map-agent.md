...
- 放置 PackedScene 对象前**必须**先用 `find_placement_anchors` 或 `validate_object_placements` 拿到真实合法坐标，禁止凭对话里"看起来像坑/像平台"去目测/编造 x/y/z——哪怕是陷阱坑、宝箱这种"肉眼很明显"的位置，也要先查，因为视觉判断的格子在目标 `map_layer` 上很可能并不是空的（被地面/装饰层占用）。树木、建筑、NPC、敌人、宝箱、陷阱/killzone 等都必须满足同一套语义摆放约束：`anchor`（默认 `bottom_center`）、footprint、`support_layers`、`forbidden_layers`、方向性 clearance（left/right/up/down/front/back）、同类最小距离、脚下/墙面/水面/空中等 `surface_type`、保护路径/房间中心/支线末端/路径边缘等设计信号。`surface_type=room_center` 时传 `room_centers`，`branch_end` 时传 `branch_ends`，`path_edge` 时传 `path_cells`/`route_cells`/`protected_cells`；工具会从这些集合中提取候选，而不是继续全区域乱搜。需要玩家能拿到奖励、接触 NPC/宝箱、进入建筑或到达敌人平台时传 `requires_reachable=true` + `start` + 正确 `movement_model`，必要时传 `reachability_point=interaction/entrance` 和 offset。`place_map_objects` 自身也会执行这些检查；如果返回 `placement_missing_support`、`placement_support_layer_mismatch`、`placement_forbidden_layer`、`placement_same_kind_too_close`、`placement_unreachable`、`placement_cell_not_empty`、`placement_clearance_blocked` 或 `placement_protected_cell`，不要换一个同样靠猜的坐标重试，改用 `find_placement_anchors` 搜附近合法锚点，且最多允许猜测重试一次——第二次仍失败就必须先查锚点。
- `place_map_objects` 的 `parent_path` 不要凭空猜节点名（例如猜成场景根节点名 "Game"）。不传 `parent_path` 时工具会自动找 `ObjectLayer`（2D）/`PropsRoot`（3D）；如果报 `object_parent_required`，说明场景里还没有这个标准层，应先调用 `ensure_standard_map_layers` 补齐，而不是手动指定一个猜测的路径或用 `parent_path="."` 把对象直接挂到地图节点本身下（这会破坏标准分层）。只有当用户明确要求挂到某个已确认存在的节点下时，才显式传 `parent_path`。
