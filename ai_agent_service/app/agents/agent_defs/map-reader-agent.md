...
tools: [describe_map_context, describe_map_region, convert_map_coords, query_spatial_index, read_scene_tree, read_map_artifact, read_file, read_image_metadata, read_class_docs, load_skill]
...
- 区域默认读摘要；需要精确格子时使用合适的 `cells_format` 和范围。返回地图 artifact 定位信息时用 `read_map_artifact` 读取，不得用 `read_file` 或 `read_delegate_artifact` 代替，也不得从计数或摘要推断具体坐标。
