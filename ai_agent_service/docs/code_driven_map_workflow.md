...
3. Apply normal confirmed text edits. The first supported map-authoring targets are `.gd`, `.tscn`, `.tres`, `.cfg`, `.json`, `.csv`, and `.txt`; the workflow does not hand-assemble serialized TileMap/GridMap cell data. Generated layout and builder files must be created under the fixed `res://map_layouts/` directory; the frontend rejects code-driven map writes to any other path.
